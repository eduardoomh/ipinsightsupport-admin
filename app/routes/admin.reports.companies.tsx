...
export const loader: LoaderFunction = async ({ request }) => { 
    ...
    const companyId = url.searchParams.get("client_id");
    const userId = url.searchParams.get("user_id");

    const apiUrl = new URL(`${process.env.APP_URL}/api/clients/report-preview`);
    ...
    if (companyId) apiUrl.searchParams.set("client_id", companyId);
...
export const meta: MetaFunction = () => [
    { title: "Company Reports | Sentinelux" },
    { name: "description", content: "Company Reports page from Sentinelux Admin" },
];

export default function AdminCompaniesReport() {
    ...
    return (
        <DashboardLayout title="Company Reports" headerActions={headerActions}>
            {isLoading ? (
                <SkeletonEntries />
            ) : (
